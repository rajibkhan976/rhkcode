...
if($stmt->num_rows > 0){
	...
	while($stmt->fetch()){
		...
		echo "<button type='button' class='btn btn-warning' id='adminupdatePostBtn' data-target='#adminUpdatedPost' data-updateid='".$pid."' data-toggle='modal'>Edit</button> ";
		echo " <button type='button' class='btn btn-danger' id='admindeletePostBtn' data-deleteid='".$pid."'>Delete</button>";
		echo "</div>";
		echo "</div>";
		}
	}
