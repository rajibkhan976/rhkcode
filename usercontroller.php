...
if(isset($_POST['login'])){
	...
	$loginquery = "select * from user where uemail = '".$logemail."' and upass = '".$logpass."'";
	$user = $mysqli->query($loginquery);
	if($user->num_rows > 0){
		echo "1";
		$userrecord = $user->fetch_object();
		$_SESSION['status'] = 'loggedin';
		$_SESSION['username'] = $userrecord->uname;
		}
		else{
			echo "0";
			}
	...
	if(isset($_POST['sel'])){
		...
		if($results->num_rows > 0){
			...
			while($row = $results->fetch_object()){
				...
				echo "<td><input type='password' value='".$row->upass."' disabled></td>";
				echo "<td><button type='button' class='btn btn-warning edituser' data-usereditid='".$row->uid."'>Edit</button>"." | "."<button type='button' class='btn btn-danger deleteuser' data-userdeleteid='".$row->uid."'>Delete</button></td>";
				echo "</tr>";
				}
				echo "</table>";
			}
		...
		if(isset($_POST['manipulate'])){
			...
			if($answer->num_rows > 0){
				...
				echo "<button type='button' class='btn btn-success' id='savebutton'>Save</button><br/> <br/>";
				echo "</form>";
				}
			}
